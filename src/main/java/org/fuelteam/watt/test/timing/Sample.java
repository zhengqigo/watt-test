...
public class Sample {
    ...
    public static void main(String[] args) {
        ...
        SlidingWindowCollection<HeartRate> swc = new SlidingWindowCollection<HeartRate>(person.getHeartRates(),
                heartRate -> heartRate.getTimestamp(), duration, every, start - 600000);
    ...
    public static void printWindowInfo(int count, TimeWindow<HeartRate> window) {
        ...
        System.out.println(String.format("%s%s%s%s%s", prefix, "Start Time - ",
                Timestamp.from(Instant.ofEpochMilli(window.getStartTime())).toString(), " End Time - ",
                Timestamp.from(Instant.ofEpochMilli(window.getEndTime())).toString()));
    }

    public static void printWindowContentInfo(HeartRate hr) {
        System.out.println(String.format("%s%s%d%s%s", prefix, "HeartRate - ", hr.getHeartRate(), " Timestamp - ",
